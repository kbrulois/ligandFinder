...
desc_colors <- c("#FED439FF", "#709AE1FF", "#D2AF81FF", "#FD7446FF",
...
assemble_anno_feats <- function(x, peps) {

  tmp <- x %>%
    as_tibble %>%
    filter(case_when(type %in% anno_feats$feature[1:3] ~ type %in% anno_feats$feature[1:3],
                     source %in% anno_feats$feature[4:6] ~ source %in% anno_feats$feature[4:6],
                     type == "peptide" & source == "uni_pep" ~ type == "peptide" & source == "uni_pep")) %>%
    mutate(feature = case_when(type %in% anno_feats$feature[1:3] ~ type,
                               source %in% anno_feats$feature[4:6] ~ source,
                               type == "peptide" & source == "uni_pep" ~ "uniprot_peptide"), .before = everything()) %>%
    mutate(end = case_when(feature == "disulfide bond" & is.na(end) & stringr::str_detect(description, "Interchain") ~ stringr::str_extract(description, "C-\\d+") %>% stringr::str_remove(., "C-") %>% as.integer(.),
                           TRUE ~ end)) %>%
    filter(!is.na(end)) %>%
...
make_protein_plot <- function(input,
  ...
  p_title <- input %>%
    ...
    mutate(`View Model` = paste0("do something"))
  ...
  seq_p <- ggplot2::ggplot(seq_dat) +
    ggiraph::geom_text_interactive(aes(x = index, y = 1, label = AA, data_id = index, onclick = paste0(index)), vjust = 0, size = 4, inherit.aes = FALSE) +

    ggplot2::geom_text(data = seq_dat,
                       aes(x = index, y = 1, label = index_tp), vjust = -2, size = 2.5, inherit.aes = FALSE) +
    ggplot2::scale_x_continuous(limits = c(0, max(80, max_index) + 1), expand = expansion(add = c(seq_offset, -0.4))) +
    ggplot2::scale_y_discrete(expand = grid::unit(0, "lines")) +
    theme_void() + theme(
  ...
  h_rec_dat <- anno_feat_dat %>%
    ...
    mutate(metric_type = "") %>%
    mutate(metric_type = factor(metric_type, levels = c("", "discrete", "blosum62\n-------------\ngrantham"))) %>%
    mutate(nudge = unname(c(`new_pep` = 2.0, `uniprot_peptide` = 1.5, `phs` = 1.25, `gpcrdb_gtp` = 0.75, `sven` = 1)[feature]))

  top_y <- df %>% filter(metric_type == "") %>% mutate(metric = droplevels(metric)) %>% pull(metric) %>% levels %>% length


  main_p <- main_p + ggplot2::geom_segment(
    data = h_rec_dat,
    aes(x = start - 0.3, xend = end + 0.3, y = top_y + nudge, yend = top_y + nudge, color = feature),
    inherit.aes = FALSE,
    linewidth = 0.8,
    lineend = "round") +
