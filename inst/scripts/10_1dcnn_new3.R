...
for(term in names(nn_input)[2]) {
  ...
  shared <- inputs |>
    ...
    layer_conv_1d(64, kernel_size = 4, padding = "same",
                  activation = "gelu", dilation_rate = 1, kernel_regularizer = regularizer)
  #layer_batch_normalization() |>
  #layer_dropout(0.3)
  ...
  masked_sum <- per_index_logits |>
    layer_lambda(function(x) {
      ...
      masked <- keras$ops$multiply(x, mask)
      keras$ops$sum(masked, axis = 2L)
    })
 ...
 # Global head (separate branch)
  ...
  early_stopping <- callback_early_stopping(
    monitor = "val_per_index_categorical_accuracy",
    patience = 20,
    mode = 'max',
    start_from_epoch = 20,
  ...
  x_all <- array(
    unlist(nn_in$all$data),
    dim = c(seq_len, n_channels, length(nn_in$all$data))
  )

  x_all <- aperm(x_all, c(3, 1, 2))
...
nn_input_comb %>% filter(known == 0 & win_type == "db") %>%
  filter(peps %in% c("ANO8_w12-47", "ANO8_w36-71", "ANO8_w14-49") | gene %in% c("ANO8", "CXCL14")) %>%
  View()
