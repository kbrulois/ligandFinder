...
class_cols <- setNames(c("#FED439FF", "#370335FF", "#8A9197FF", "#D2AF81FF",
                       ...
                       c("CT_cleavage_context", "DB", "gap", "NT_cleavage_context", "pep_other", "pep_pocket", "none")
)
...
for(term in names(nn_input)[1:2]) {
  ...
  # Encoder
  conv1 <- inputs |>
    layer_conv_1d(32, kernel_size = 8, padding = "same", activation = "gelu",
                  kernel_regularizer = regularizer)

  pool1 <- conv1 |> layer_average_pooling_1d(pool_size = 2)  # 36 -> 18

  conv2 <- pool1 |>
    layer_conv_1d(16, kernel_size = 4, padding = "same", activation = "gelu",
                  kernel_regularizer = regularizer) |>
    layer_dropout(0.2)

  pool2 <- conv2 |> layer_average_pooling_1d(pool_size = 2)  # 18 -> 9

  conv3 <- pool2 |>
    layer_conv_1d(16, kernel_size = 2, padding = "same", activation = "gelu",
                  kernel_regularizer = regularizer) |>
    layer_dropout(0.4)

  # Decoder - upsample back to 36
  # Decoder
  up1 <- conv3 |> layer_upsampling_1d(size = 2)  # 9 -> 18

  up2 <- layer_concatenate(list(up1, conv2)) |>   # both 16 filters ✓
    layer_conv_1d(16, kernel_size = 4, padding = "same", activation = "gelu",
                  kernel_regularizer = regularizer)

  up3 <- up2 |> layer_upsampling_1d(size = 2)    # 18 -> 36

  # Project conv1 down to 16 filters before concatenating
  conv1_proj <- conv1 |>
    layer_conv_1d(16, kernel_size = 1)            # 32 -> 16 filters

  shared <- layer_concatenate(list(up3, conv1_proj)) |>  # both 16 filters ✓
    layer_conv_1d(16, kernel_size = 8, padding = "same", activation = "gelu",
                  kernel_regularizer = regularizer)
  ...
  masked_focal_loss <- function(y_true, y_pred, gamma = 4.0, smoothness_weight = 1) {
...
View(nn_input_comb %>% filter(known == 0 & target == "C"))
