...
ggplot2::ggplot(secretome_aa %>% mutate(known_peptide2 = ifelse(known_peptide == 1, "known", "unknown"))) +
  ggplot2::geom_boxplot(aes(x = known_peptide2, y = `AF_Psi_cos`)) 
...
nn[["nn5"]] <- c(nn[["nn2"]], "^SS_")
nn[["nn6"]] <- c(nn[["nn2"]], "AF_Phi_cos", "AF_Psi_cos", "AF_Phi_sin", "AF_Psi_sin")
nn[["nn7"]] <- c(nn[["nn5"]], "AF_Phi_cos", "AF_Psi_cos", "AF_Phi_sin", "AF_Psi_sin")
nn[["nn8"]] <- c(nn[["nn7"]], "_energy$")
