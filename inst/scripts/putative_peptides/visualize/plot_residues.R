...
secretome_aa <- readRDS("~/peptide_alg/secretome_aa_Dec11.rds")
...
furrr::future_map(names(plot_dat), \(x) {
  ...
  if("AA" %in% names(cons_dat)) {
    to_plot <- bind_cols(pd[["dssp"]][[1]], cons_dat %>% dplyr::select(-index, -AA))
  } else {
    to_plot <- pd[["dssp"]][[1]] %>%
      mutate(cons = NA,
             frequency = NA,
  ...
  if("mean_af_missense" %in% colnames(pd$af_missense[[1]])) {
    p_afm_dat <- pd$af_missense[[1]] %>%
  ...
  if("AA" %in% colnames(pd$msa_mean[[1]])) {
    p_msa_mean_dat <- pd$msa_mean[[1]] %>%
      mutate(index = 1:nrow(.), .before = everything()) %>%
      dplyr::select(-AA) %>%
      mutate(AA_mean = scales::rescale(AA_mean, to = c(0,1))) %>%
      reshape2::melt(id.vars = "index", variable.name = "msa")
  } else {
    p_afm_dat <- tibble(index = 1:p_size,
                        afm = as.numeric(rep(NA, p_size)),
                        value = as.numeric(rep(NA, p_size)))
  ...
  p_msa_mean <- ggplot2::ggplot(p_msa_mean_dat) +
    ggplot2::geom_tile(aes(x = index, y = msa, fill = value), show.legend = FALSE) +
    ggplot2::scale_fill_viridis_c(option = "H") +
    scale_x_continuous(expand = expansion(mult = buffer)) +
    ylab("per residue msa") +
    ggplot2::theme_bw()

  if("AA" %in% colnames(pd$msa_sd[[1]])) {
    p_msa_sd_dat <- pd$msa_sd[[1]] %>%
      mutate(index = 1:nrow(.), .before = everything()) %>%
      dplyr::select(-AA) %>%
      mutate(AA_mean = scales::rescale(AA_mean, to = c(0,1))) %>%
      reshape2::melt(id.vars = "index", variable.name = "msa")
  ...
  p_msa_sd <- ggplot2::ggplot(p_msa_sd_dat) +
    ggplot2::geom_tile(aes(x = index, y = msa, fill = value), show.legend = FALSE) +
    ggplot2::scale_fill_viridis_c(option = "H") +
    scale_x_continuous(expand = expansion(mult = buffer)) +
    ylab("per residue msa (sd)") +
    ggplot2::theme_bw()



  p_feature_dat <-  plot_dat[[x]] %>%
    ungroup %>%
    unnest(c("cons", "dssp"), names_sep = "_") %>%
    dplyr::select(starts_with("cons_index"),
                  starts_with("cons_frequency_scaled"),
                  starts_with("dssp_relASA"),
                  any_of(params_tp)) %>%
    reshape2::melt(id.vars = "cons_index", variable.name = "params") %>%
  ...
  p_feat_dat <- ggplot2::ggplot(p_feature_dat) +
    ggplot2::geom_tile(aes(x = cons_index, y = params, fill = value), show.legend = x == "parameter") +
